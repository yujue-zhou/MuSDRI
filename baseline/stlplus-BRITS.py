...
# hyper-parameters
parser = argparse.ArgumentParser(description='stlplus-BRITS algorithm on GAS dataset')
...
parser.add_argument('--freq_STL', default=84, type=int, help='parameter frequence for STL algorithm') 
...
train_remainder = pd.read_csv('./data/train/stlplus_train_remainder'+str(args.freq_STL)+'.csv', index_col=0)
train_seasonal = pd.read_csv('./data/train/stlplus_train_seasonal'+str(args.freq_STL)+'.csv', index_col=0)
train_trend = pd.read_csv('./data/train/stlplus_train_trend'+str(args.freq_STL)+'.csv', index_col=0)


train_dataset_value= train_remainder.to_numpy()*train_org_mask.to_numpy()*train_mask.to_numpy()
...
train_dataset = np.concatenate((train_dataset_value,train_dataset_mask,train_dataset_deltas, train_dataset_value[:,:,::-1,:], train_dataset_mask[:,:,::-1,:], train_dataset_deltas[:,:,::-1,:]),1)
...
test_remainder = pd.read_csv('./data/test/stlplus_test_remainder'+str(args.freq_STL)+'.csv', index_col=0)
test_seasonal = pd.read_csv('./data/test/stlplus_test_seasonal'+str(args.freq_STL)+'.csv', index_col=0)
test_trend = pd.read_csv('./data/test/stlplus_test_trend'+str(args.freq_STL)+'.csv', index_col=0)


test_null_number =test_data_null.isna().sum().sum()-test_data_statiton.isna().sum().sum()

test_dataset_value= test_remainder.to_numpy()*test_org_mask.to_numpy()*test_mask.to_numpy()
...
test_dataset = np.concatenate((test_dataset_value,test_dataset_mask,test_dataset_deltas, test_dataset_value[:,:,::-1,:], test_dataset_mask[:,:,::-1,:], test_dataset_deltas[:,:,::-1,:]),1)
...
for epoch in range(20):
  ...
  for index, data in enumerate(trainloader):
    ...
    if index%10==0:
      ...
      Y_predicted = Y_predicted+test_seasonal+test_trend
      ...
      file = open("./losses/stlplus-BRITS-"+str(args.freq_STL)+".txt","a")
      ...
      if test_loss<best_loss:
        ...
        save_point = './checkpoint/stlplus-BRITS-'+str(args.freq_STL)
        torch.save(model_state, save_point)
        

print('finished training')
checkpoint = torch.load('./checkpoint/stlplus-BRITS-'+str(args.freq_STL))
...
file.write('stlplus-BRITS\n')
file.write('seq_len = {},   '.format(seq_len))
file.write('frequence = {}   '.format(args.freq_STL))
file.write('\n')
...
imputed_data.to_csv('./data/test/impute/stlplus-BRITS-'+str(args.freq_STL)+'.csv',index=0)
