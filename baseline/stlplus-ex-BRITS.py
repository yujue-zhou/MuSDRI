...
# hyper-parameters
parser = argparse.ArgumentParser(description='stlplus-exogenous-BRITS algorithm on GAS dataset')
...
class rits(nn.Module):
    ...
    def forward(self, data, direct):
        ...
        for t in range(data.shape[2]):
            ...
            inputs = torch.cat([c_c, m, e, s], dim = 1)
...
train_seasonality = pd.read_csv('./data/train/stlplus_train_seasonal'+str(args.freq_STL)+'.csv', index_col=0)
...
train_trend = train_trend.to_numpy()
train_trend = incision(train_trend, seq_len)

train_seasonality = train_seasonality.to_numpy()
train_seasonality = incision(train_seasonality, seq_len)


train_dataset = np.concatenate((train_dataset_value,train_dataset_mask,train_dataset_deltas, 
  train_trend, train_seasonality,
  train_dataset_value[:,:,::-1,:], train_dataset_mask[:,:,::-1,:], train_dataset_deltas[:,:,::-1,:],
  train_trend[:,:,::-1,:], train_seasonality[:,:,::-1,:]
  ),1)
...
test_seasonality = pd.read_csv('./data/test/stlplus_test_seasonal'+str(args.freq_STL)+'.csv', index_col=0)
...
test_trend = test_trend.to_numpy()
test_trend = incision(test_trend, len(test_data_statiton))

test_seasonality = test_seasonality.to_numpy()
test_seasonality = incision(test_seasonality, len(test_data_statiton))


test_dataset = np.concatenate((test_dataset_value,test_dataset_mask,test_dataset_deltas, 
  test_trend, test_seasonality,
  test_dataset_value[:,:,::-1,:], test_dataset_mask[:,:,::-1,:], test_dataset_deltas[:,:,::-1,:],
  test_trend[:,:,::-1,:], test_seasonality[:,:,::-1,:]
  ),1)
...
for epoch in range(20):
  ...
  for index, data in enumerate(trainloader):
    ...
    if index%10==0:
      ...
      file = open("./losses/stlplus-exogenous-BRITS-"+str(args.freq_STL)+".txt","a")
      ...
      if test_loss<best_loss:
        ...
        save_point = './checkpoint/stlplus-exogenous-BRITS-'+str(args.freq_STL)
        torch.save(model_state, save_point)
        

print('finished training')
checkpoint = torch.load('./checkpoint/stlplus-exogenous-BRITS-'+str(args.freq_STL))
...
file.write('stlplus-exogenous-BRITS\n')
...
imputed_data.to_csv('./data/test/impute/stlplus-exogenous-BRITS-'+str(args.freq_STL)+'.csv',index=0)
