...
# hyper-parameters
parser = argparse.ArgumentParser(description='mstlplus-exogenous-BRITS algorithm on GAS dataset')
...
class rits(nn.Module):
    ...
    def build(self):
        self.rnn_cell = nn.LSTMCell(args.num_features*5, self.rnn_hid_size)
    ...
    def forward(self, data, direct):
        ...
        for t in range(data.shape[2]):
            ...
            inputs = torch.cat([c_c, m, e, s1, s2], dim = 1)
...
train_seasonality1 = pd.read_csv('./data/train/mstlplus_seasonal12.csv', index_col=0)
train_seasonality2 = pd.read_csv('./data/train/mstlplus_seasonal84.csv', index_col=0)
...
train_dataset = np.concatenate((train_dataset_value,train_dataset_mask,train_dataset_deltas, 
  train_trend, train_seasonality1, train_seasonality2, 
  train_dataset_value[:,:,::-1,:], train_dataset_mask[:,:,::-1,:], train_dataset_deltas[:,:,::-1,:],
  train_trend[:,:,::-1,:], train_seasonality1[:,:,::-1,:], train_seasonality2[:,:,::-1,:]
  ),1)
...
test_seasonality1 = pd.read_csv('./data/test/mstlplus_seasonal12.csv', index_col=0)
test_seasonality2 = pd.read_csv('./data/test/mstlplus_seasonal84.csv', index_col=0)
...
test_dataset = np.concatenate((test_dataset_value,test_dataset_mask,test_dataset_deltas, 
  test_trend, test_seasonality1, test_seasonality2,
  test_dataset_value[:,:,::-1,:], test_dataset_mask[:,:,::-1,:], test_dataset_deltas[:,:,::-1,:],
  test_trend[:,:,::-1,:], test_seasonality1[:,:,::-1,:], test_seasonality2[:,:,::-1,:]
  ),1)
...
for epoch in range(20):
  ...
  for index, data in enumerate(trainloader):
    ...
    if index%10==0:
      ...
      file = open("./losses/mstlplus-exogenous-BRITS.txt","a")
      ...
      if test_loss<best_loss:
        ...
        save_point = './checkpoint/mstlplus-exogenous-BRITS'
        torch.save(model_state, save_point)
        

print('finished training')
checkpoint = torch.load('./checkpoint/mstlplus-exogenous-BRITS')
...
file.write('mstlplus-exogenous-BRITS\n')
...
imputed_data.to_csv('./data/test/impute/mstlplus-exogenous-BRITS.csv',index=0)
