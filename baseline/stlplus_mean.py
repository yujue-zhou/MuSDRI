...
data_stlplus_mean = remainder_mean.to_numpy()+trend_matrix+seasonal_matrix


error_mask = (test_data_statiton.fillna(0).to_numpy()-data_stlplus_mean)*(1-test_mask).to_numpy()
...
data_stlplus_mean = pd.DataFrame(data_stlplus_mean)
data_stlplus_mean.columns = test_data_statiton.columns.tolist()
data_stlplus_mean.to_csv('./data/test/impute/stlplus_mean'+str(freq)+'.csv',index=0)


file = open('results_MAE_MRE_MSE.txt','a')
file.write('stlplus_mean \n')
